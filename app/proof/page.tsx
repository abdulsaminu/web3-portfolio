...
// Your transaction-backed evidence
const proofs = [
  ...
  return (
    <div className="container-main py-10">
      <div className="fade-up mb-10">
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight">🔍 On‑Chain Proof</h1>
        <p className="text-gray-400 text-lg mt-1">Every card = real transaction. No fakes.</p>
      </div>
      ...
      <div className="grid-tight">
        {filtered.map((proof, idx) => (
          <div key={idx} className="card fade-up" style={{ animationDelay: `${idx * 0.05}s` }}>
            ...
            <div className="relative h-44 w-full mb-4 rounded-lg overflow-hidden bg-gray-900 border border-white/5">
              <Image src={proof.image} alt={proof.title} fill className="object-cover hover:scale-105 transition duration-300" />
            </div>
            <h3 className="text-xl font-bold tracking-tight">{proof.title}</h3>
            <p className="text-gray-400 text-sm mt-1 leading-relaxed">{proof.description}</p>
            <a href={proof.txLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 mt-4 text-accent text-sm font-medium hover:text-white">
              📜 View Transaction →
            </a>
          </div>
        ))}
      </div>
      <div className="text-center text-gray-500 text-sm mt-12 pt-6 border-t border-white/5">
        All transactions verifiable on Arc testnet. Questions? <a href="https://twitter.com/abdul_sd01" target="_blank" className="text-accent">@abdul_sd01</a>
      </div>
    </div>
  );
}